...
export default async function NewProductPage() {
  ...
  return <div className="management-page management-product-page"><Link className="management-back" href="/gestion/catalogo">← Productos</Link><header className="management-page-header"><div><h1>Nuevo producto</h1><p>Completá la información y las variantes. Luego podrás cargar las imágenes.</p></div></header>{categories.results.length ? <ProductEditorPanel attributes={attributes.results} brands={brands.results} categories={categories.results} /> : <div className="management-empty"><h2>Primero creá una categoría</h2><Link className="button primary" href="/gestion/categorias">Crear categoría</Link></div>}</div>;
}
