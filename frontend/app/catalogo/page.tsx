...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><SiteHeader categories={categories} /><main id="contenido" className="page-shell shell"><nav className="breadcrumb" aria-label="Migas de pan"><Link href="/">Inicio</Link><span>/</span><span>Catálogo</span></nav>{failed ? <><div className="catalog-title"><h1>{initial.q ? `Resultados para “${initial.q}”` : "Todo el catálogo"}</h1><p>Elegí por categoría, marca, precio y disponibilidad.</p></div><div className="empty-state"><h2>No pudimos mostrar el catálogo</h2><p>Reintentá en unos minutos. Tus filtros van a seguir aplicados.</p><a className="button primary" href={`/catalogo${buildCatalogQuery(initial) ? `?${buildCatalogQuery(initial)}` : ""}`}>Reintentar</a></div></> : <CatalogBrowser response={response} categories={categories} initial={initial} />}</main><SiteFooter /></>;
}
