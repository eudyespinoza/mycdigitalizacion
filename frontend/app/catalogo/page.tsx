...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><SiteHeader categories={categories} /><main id="contenido" className="page-shell shell"><nav className="breadcrumb" aria-label="Migas de pan"><Link href="/">Inicio</Link><span>/</span><span>Catálogo</span></nav><div className="catalog-title"><h1>{initial.q ? `Resultados para “${initial.q}”` : "Todo el catálogo"}</h1><p>Buscá y filtrá con disponibilidad, precios y atributos publicados por la tienda.</p></div>{failed ? <div className="empty-state"><h2>No pudimos cargar el catálogo</h2><p>La tienda no respondió. Tus filtros siguen en la URL.</p><a className="button primary" href={`/catalogo${buildCatalogQuery(initial) ? `?${buildCatalogQuery(initial)}` : ""}`}>Reintentar</a></div> : <CatalogBrowser response={response} categories={categories} initial={initial} />}</main><SiteFooter /></>;
}
