...
test("registration sends the complete storefront profile and focuses the first invalid field", async ({ page, request }, testInfo) => {
  test.skip(testInfo.project.name !== "360");
...
test("login rotates CSRF and profile persists the masked DNI before checkout", async ({ page, request }, testInfo) => {
...
test("address workflow supports postal lookup, near confirmation and explicit far reverse choice", async ({ page, request }, testInfo) => {
  test.skip(testInfo.project.name !== "1024");
...
test("mobile catalog sheet traps focus and server URL state produces removable chips", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "768");
...
test("CMS failure is distinct from empty content and leaves catalog recovery available", async ({ page, request }, testInfo) => {
  test.skip(testInfo.project.name !== "360");
...
test("checkout reviews authoritative data and stops at server pending review", async ({ page, request }, testInfo) => {
  test.skip(testInfo.project.name !== "1440");
...
test("payment result polling is bounded, manually retryable, and order detail uses server timeline", async ({ page, request }, testInfo) => {
  ...
  await control(request, { payments: ["approved"] });
...
test("accessible interaction colors meet required contrast", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "1440");
  await page.goto("/");
  const ratios = await page.evaluate(() => {
    const parse = (hex: string) => [1, 3, 5].map((index) => Number.parseInt(hex.slice(index, index + 2), 16) / 255);
    const lum = (hex: string) => parse(hex).map((value) => value <= 0.03928 ? value / 12.92 : ((value + 0.055) / 1.055) ** 2.4).reduce((sum, value, index) => sum + value * [0.2126, 0.7152, 0.0722][index], 0);
    const ratio = (a: string, b: string) => { const [light, dark] = [lum(a), lum(b)].sort((x, y) => y - x); return (light + 0.05) / (dark + 0.05); };
    const styles = getComputedStyle(document.documentElement);
    return { primary: ratio(styles.getPropertyValue("--magenta-action").trim(), "#ffffff"), focus: ratio(styles.getPropertyValue("--cyan-action").trim(), "#ffffff") };
  });
