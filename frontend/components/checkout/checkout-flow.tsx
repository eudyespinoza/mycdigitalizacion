...
import type { Address, BillingProfile, CheckoutResponse, Customer, IdentityStatus, ShippingQuote } from "@/lib/types";

export type CheckoutUiState = "idle" | "checking" | "pending_review" | "provider_down" | "approved" | "rejected" | "needs_attention";

export function getTrustedOrderState(_params: URLSearchParams, serverPaymentStatus: string | null): CheckoutUiState {
  if (!serverPaymentStatus || ["pending", "in_process"].includes(serverPaymentStatus)) return "checking";
  if (["approved", "paid"].includes(serverPaymentStatus)) return "approved";
...
export function CheckoutFlow() {
  ...
  const recover = (cause: unknown, fallback: string) => {
    if (cause instanceof ApiError) {
      const recovery = checkoutRecoveryFor(cause.code);
  ...
  const loadAccount = async () => {
    ...
    try {
      const [me, identityStatus, addressRows, billingRows] = await Promise.all([
      ...
      setCustomer(me); setIdentity(identityStatus); setAddresses(addressRows); setBilling(billingRows);
      ...
      if (!me.email_verified_at) throw new ApiError(422, "email_unverified", "Email pendiente");
    ...
    } catch (cause) { recover(cause, "Iniciá sesión y completá tu perfil para continuar."); }
  ...
  const validateIdentity = async () => {
    ...
    } catch (cause) { recover(cause, "No pudimos validar la identidad."); }
    finally { setBusy(false); }
  };
  ...
  const getQuote = async () => {
    ...
    catch (cause) { recover(cause, "No pudimos cotizar el envío."); }
    finally { setBusy(false); }
  };
  const confirm = async () => {
    if (!cartContext?.cart?.lines.length) { recover(new ApiError(422, "empty_cart", "Carrito vacío"), "El carrito está vacío."); return; }
    setBusy(true); setError("");
    try {
    ...
    } catch (cause) { recover(cause, "No pudimos iniciar el pago."); }
    finally { setBusy(false); }
  };
