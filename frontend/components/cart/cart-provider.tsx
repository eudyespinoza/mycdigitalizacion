...
type CartContextValue = { cart: Cart | null; loading: boolean; open: boolean; error: string; setOpen: (open: boolean) => void; restoreFocus: () => void; refresh: () => Promise<void>; add: (payload: { variant_id: number; quantity: number }) => Promise<void>; setQuantity: (variant_id: number, quantity: number) => Promise<void>; applyCoupon: (coupon: string) => Promise<void>; remove: (variant_id: number) => Promise<void> };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<CartContextValue>(() => ({ cart, loading: pending > 0, open, error, setOpen, restoreFocus, refresh, add: async (payload) => { const active = document.activeElement instanceof HTMLElement ? document.activeElement : null; await perform(() => cartApi.add(payload, token())); opener.current = active; setOpenState(true); }, setQuantity: (variant_id, quantity) => perform(() => cartApi.quantity({ variant_id, quantity }, token())), applyCoupon: (coupon) => perform(() => cartApi.coupon(coupon, token())), remove: (variant_id) => perform(() => cartApi.clear(variant_id, token())) }), [cart, pending, open, error, perform, refresh, restoreFocus, setOpen]);
